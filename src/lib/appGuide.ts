...
export function appGuide(): string {
  ...
  return [
    ...
    "• 🔗 INTEGRATIONS — Connect Google (one tap grants read-only Gmail, send-email, and read/write Calendar). Gmail: “Scan inbox now” turns real action items in recent mail into commitments; in Capture you can also say “email X about …” and the agent drafts a full email for you to review and send (it never auto-sends). Calendar: upcoming events show in the brief, and the assistant can add events. Connect GitHub with a read-only token to distill your recent commits into an engineering note.",
