...
const financialDecision: TypeConfig = {
  ...
  fields: [
    ...
    { key: "falsifier", label: "What would make this a mistake", kind: "textarea", placeholder: "The outcome that would mean this was the wrong call" },
...
const goal: TypeConfig = {
  ...
  fields: [
    ...
    { key: "monthly", label: "Monthly contribution", kind: "number", placeholder: "10000", help: "Leave blank to auto-sum the SIPs you link to this goal." },
    { key: "expectedReturn", label: "Expected annual return (%)", kind: "number", placeholder: "10", help: "For projecting growth. Blank assumes 10% if you're contributing." },
...
export const moneyModule: ModuleConfig = {
  ...
  agentHint:
    "Money capture — always extract the amount. A significant money choice to grade later (confidence + expected value) → financial-decision; everyday spending worth remembering (capture satisfaction if the user implies it) → expense; money put to work with a thesis (SIP, stock, fund, education, business) → investment; a savings target → goal. For investments: create ONE entry PER fund/asset (e.g. flexi cap and small cap are TWO separate investments), set frequency=monthly for a monthly SIP with amount = the monthly installment, and capture the thesis. A savings/wealth TARGET like '₹10L by 2030' is a `goal`, NEVER a `project` — projects are efforts/work, goals are money targets. When the user gives a goal PLUS the SIPs/funds that fund it, create the goal AND one investment per fund as SEPARATE entries (never merge them into one project). A money *lesson* learned is a core `lesson` tagged money — not a money type. Keep follow-up questions minimal.",
};
